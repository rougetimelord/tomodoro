...
function keyPress(event, c)
{
    ...
    if (c == 11) {
        if (run == true) {
            var swischer = false;
            flasher = setInterval(function () { var img = (swischer == false) ? "/tomodoro/Content/heman.gif" : "/tomodoro/Content/frog.png"; document.body.style.background = "url('" + img + "') repeat right top"; swischer = !swischer; }, 2500);
        }
        if(run == false) {
